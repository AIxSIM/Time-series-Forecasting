...
loss_history = train_autoformer(autoformer, train_loader, test_loader, encoder, autoformer_optimizer, sequence_length, num_features, scaler, epochs=1)
...
# Save the model
save_model(autoformer, "autoformer_model_"+datetime.now().strftime("%Y%m%d%H%M")+".pth")
